...
def main():
    ...
    parser.add_argument("--max-tokens", type=int, default=1024,
                        help="max_tokens for all prompts (caption/JSON/PPE). default: 1024")
    ...
    # === Caption ===
    print(f"=== Caption Generation (max_tokens={args.max_tokens}) ===")
    for img in tech_images:
        name = os.path.basename(img)
        print(f"  {name} ...", end=" ", flush=True)
        r = call_vlm(client, args.api_url, args.model, img, PROMPT_CAPTION, max_tokens=args.max_tokens)
        r["image"] = name
        results["captions"].append(r)
        print(f"{r['latency']}s, {r.get('tok_s',0)} tok/s")

    # === JSON Extraction ===
    print(f"\n=== JSON Tag Extraction (max_tokens={args.max_tokens}) ===")
    for img in tech_images:
        name = os.path.basename(img)
        print(f"  {name} ...", end=" ", flush=True)
        r = call_vlm(client, args.api_url, args.model, img, PROMPT_JSON, max_tokens=args.max_tokens)
        r["image"] = name
    ...
    # === PPE Detection ===
    print(f"\n=== PPE Detection (max_tokens={args.max_tokens}) ===")
    for img in ppe_images:
        name = os.path.basename(img)
        print(f"  {name} ...", end=" ", flush=True)
        r = call_vlm(client, args.api_url, args.model, img, PROMPT_PPE, max_tokens=args.max_tokens)
        r["image"] = name
        r["json_parse_ok"] = try_parse_json(r.get("content", ""))
